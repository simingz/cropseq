...
nsc0507 <- data.frame(fread('data/cell_ranger_from_Alan/NSC0507.csv',sep=','),row.names=1)
colnames(nsc0507) = paste(colnames(nsc0507),'0507') #prevent overlapping barcodes

nsc08 <- data.frame(fread('data/cell_ranger_from_Alan/NSC08.csv',sep=','),row.names=1)
...
```{r}
gRNAs = readLines('data/CROP_data_from_Alan/gRNAs.txt')
gRNA.dge = comb[gRNAs,]
```
